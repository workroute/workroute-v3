...
- **Basic invoicing:** "Send bill" button, Stripe payment link on job completion.
...
1. Deploy the app somewhere public (Vercel is the natural fit, given the Next.js stack)
...
Not urgent — deliberately deferred until local testing feels reliable, per the founder's own call.
...
## 31. Generalized Pricing Configuration (Design Task — Needs Proper Session with Claude Code)

**The problem:** §26's Instant Estimate Calculator (in the locked spec) currently only exists for Lawn Mowing. With §29 adding 9 more trades, and §25's new adaptive voice AI needing real-time deterministic pricing to work at all, pricing configuration needs to generalize across trades — not be rebuilt bespoke each time a new trade is added.

**Starting architecture proposal (a strawman for Claude Code to refine against the real codebase, not a final decision):** tie pricing rules directly to each trade's existing question set (§8a/§29), rather than building a separate pricing system:
...
This means one generic "Pricing Setup" screen pattern reusable across every trade — showing that trade's actual questions with a price box next to each option — rather than a bespoke calculator per trade. When a tradie enables a new trade, they fill in prices against questions that already exist, not learn a new system each time.

**Why this matters beyond just Lawn Mowing:** this is the shared foundation both §25 (voice AI needs a price to quote live) and any future trade's estimate calculation depend on — worth designing once, properly, rather than accumulating one-off pricing logic per trade.

**Founder's note:** there may be an existing note/document with more specific thinking on this that hasn't been located yet — worth checking before finalizing the design, in case it changes or adds to the approach above.

**Status: not yet built, needs a proper design session with Claude Code** (same treatment as the Messenger got) — not something to build casually alongside everything else queued tonight.
