...
3. **Exception — VIP/known-contact override:** ~~the tradie can designate specific numbers (family, key regular customers) that bypass the AI entirely and ring straight through to the tradie's phone, same as a normal call.~~ **Superseded by §34a:** live bypass turned out to be technically unworkable (unconditional forwarding loop) — VIP numbers still route to the AI, but the tradie now gets an instant SMS heads-up instead. Configurable in settings.
...
**Voice selection (unchanged from original decision):** start with Twilio's built-in text-to-speech for simplicity while proving out the core flow; the custom ElevenLabs voice ("Emma Lilliana") remains a planned later upgrade. ~~Whichever voice is used, it must not introduce itself with a name or persona (e.g. "Sarah") — stays consistent with §23.~~ **Superseded by §37:** the no-persona-name rule is reversed — the AI now introduces itself as "Sarah, the AI Office Manager," pairing the name with explicit AI disclosure instead of avoiding a name altogether.
...
## 34a. VIP Caller SMS Alert (Added — Replaces Live VIP Call Routing, §5.1/§25)

**Resolves an open implementation gap found while planning §25 (phone voice AI):** §5.1's original VIP/known-contact override description — "bypass the AI entirely, ring straight through to the tradie's phone" — turns out to hit a real technical blocker. Once the tradie's phone has **unconditional** call forwarding active (required by §5.1's AI-answers-by-default model), any attempt to transfer a VIP call back to that same number just gets forwarded away again by the carrier — an infinite loop, not a working bypass. There's no clean way to make a VIP call actually ring the tradie's real handset without either a second phone number or in-app/WebRTC calling infrastructure, and neither is worth building for Phase 1 just for this.

**Simpler decision, replacing the live-bypass description in §5.1:** VIP calls still go to the AI like every other call — but the moment a call arrives from a pre-registered number, the tradie gets an **instant SMS heads-up** (e.g. "Sarah is calling — you might want to call them back") via the existing SMS system (§13, Mobile Message), so they know to check in even though the AI is answering live. The AI's own escalation behavior (`flag_for_attention`, same pattern as §24) remains the backstop if anything on the call genuinely needs a human.

**What this needs, small:**
- A settings field where the tradie can register a short list of "always alert me" numbers (name + phone number)
- A caller-ID check against that list when a call comes in — if matched, fire the SMS in parallel with the AI answering; must not block or delay call pickup

**Not built:** true call bypass / live transfer to the tradie's phone — punted, per the reasoning above. Worth revisiting only if in-app calling is ever built for other reasons.

## 34b. Phone AI Coverage for Non-Lawn-Mowing Trades — Resolved (§25)

**Resolves an open scoping question found while planning §25:** §25's "Lawn Mowing only" rollout line describes where the *live, AI-calculated price* experience applies — it does not mean the phone AI itself is switched off for every other trade. **Decision: the adaptive AI answers and captures job info for every trade**, using that trade's existing question set (§8a/§29) the same way it does for Lawn Mowing. The only difference is what happens at the pricing step: for Lawn Mowing, the deterministic pricing engine (§26) returns a live number; for every other trade not yet rolled out, the AI never attempts a price — the job is simply created as **Quote Required**, same honest fallback §9 already uses elsewhere in the product.

**Correction to the original reasoning (the underlying engine, not per-trade, was the false constraint):** the pricing engine itself is not Lawn-Mowing-specific — §31's generalization (`trade_pricing_configs` + `computeEstimate()`) already works generically over any trade's question set. The real, permanent distinction is not "no engine exists yet" but **which trades are honestly quotable from a phone conversation at all**:
- **Genuinely quotable trades** (Lawn Mowing, and later Pool Cleaning, Window Cleaning, Painting, Home Cleaning — clean, boundable variables) — these are candidates for the live-price experience as each gets its question set configured with rates, same pattern §25 already describes: one trade at a time, as each is proven working well.
- **Genuinely hard-to-quote-remotely trades** (Plumbing, Electrical, and similar — a "blocked drain" could reasonably be $150 or $800 depending on cause) — **Quote Required is the permanent, correct design choice for these, not a temporary gap to be closed later.** No amount of pricing-engine generalization changes that; the honest answer for these trades is a human quote, by design.

**Why this matters for today's build:** doesn't change the actual scope — starting with Lawn Mowing alone still makes sense, since it's the only trade currently configured with question-set rates. But the reasoning for *why* other trades stay Quote Required needs to be accurate going forward: it's a per-trade quotability judgment, not an engine limitation, so expansion decisions later should be made trade-by-trade on that basis.

## 37. Phone AI Greeting — Named Persona with Explicit AI Disclosure (Reverses Part of §23, §25)

**Decision, made after checking the legal question directly** (does using a persona name risk impersonating a real person): the AI never claims or implies it's human — the name is paired with explicit, upfront AI disclosure, which removes the impersonation concern entirely rather than avoiding a name to sidestep it.

**The greeting, exactly:** "[Good morning/afternoon/evening], you've reached [Business Name]. I'm Sarah, the AI Office Manager. How can I help you today?"

**Must genuinely check the actual time of the call** — not a hardcoded greeting. The time-of-day portion is computed from the real clock at the moment the call comes in, same principle as the rest of the product never inventing facts it can check.

**What stays unchanged:** §23's separate rule that "WorkRoute" itself is never mentioned still applies in full — Sarah is presented as this business's own AI Office Manager, not WorkRoute's product. Only the specific "no name, no persona" instruction from §23/§25 is reversed; the broader white-labeling principle is not.
