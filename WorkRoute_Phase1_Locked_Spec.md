...
## 35. §25 Real-World Course Correction — Live Pricing Conversation Dropped (Found Through Actual Phone Testing)

**A real test call revealed something planning couldn't have caught:** having the AI conduct a live, conversational pricing Q&A during the call — asking follow-up questions and calculating a price in real-time voice conversation — was genuinely awkward as an actual phone interaction, "fighting the medium" rather than working with it. This is exactly the value of testing early rather than over-building on assumptions.

**Revised default flow for new enquiries:** the AI captures name, contact number, and a quick description of the job (what + roughly where) — no live pricing conversation at all. It thanks the caller, confirms the tradie will be in touch, and creates the job. **The price estimate still gets calculated** — silently, in the background, from whatever was captured — so it's sitting ready for the tradie to review, but the caller never has to sit through a Q&A just to produce a number.

**Returning customer flow (new, added):** a caller recognized by matching caller ID to an existing client record gets **put straight into a created job, no scheduling conversation** — for a repeat/same-type job, this skips intake entirely rather than re-asking questions already on file.

**This changes, not just refines, part of §25's original design** — the "live price during the call" experience (e.g. "Based on what you've told me, the price would be approximately $185, would you like me to book that?") is dropped as the default interaction, in favor of simple, fast capture with pricing happening quietly afterward. The underlying safety architecture (§34b's deterministic pricing gate, the audit trail, never inventing facts) is unchanged — this is a simplification of the conversation flow, not a change to the safety model.

## 36. §25 Final Simplification — Fixed Four-Question Script (Supersedes Adaptive Question Flow)

**Further simplified, based directly on real-world testing and a clear, correct guiding principle: reliably capturing name, number, address, and a basic idea of the job already beats a missed call — that's the actual bar, not perfect structured data.**

**The phone AI now asks exactly four fixed questions, every time, no adaptive branching or trade identification during the call:**
1. "What is your name?"
2. "What's the address?"
3. "What's the best number to reach you?"
4. "Tell me briefly what you need help with."

**This replaces §25's original "adaptive conversation, dynamically loads trade-specific questions" design** — that approach added real conversational complexity that direct testing showed fought against how a phone call actually works. Four short, always-the-same questions are dramatically simpler for the AI to execute reliably and far less likely to frustrate a caller than an adaptive, branching interrogation.

**What happens to the researched trade-specific question sets (§8a, §29) — not wasted, repurposed:** the free-text answer to "what do you need help with" becomes the input for the background structured-extraction step already agreed in §35/this section — the AI can still attempt to pull out structured details (e.g. "overgrown, sloped backyard" → Size/Slope fields) from that one answer, quietly, after the call, feeding the pricing engine on a best-effort basis. The trade-specific question sets remain fully used elsewhere — the Messenger, the web/SMS intake form — this change is specific to the live phone conversation only.

**No confidence-checking or escalation logic needed around imperfect extraction** — if structured details can't be reliably extracted, that's fine; a real name, number, address, and job description already represents a genuine, valuable capture. The tradie can follow up directly for anything unclear, which is still a dramatically better outcome than a missed call or a frustrated hang-up.

## 37. Phone AI Greeting — Reverses "No Name" Rule, Replaces With Explicit AI Disclosure Instead

**Deliberate, researched reversal of part of §23/§25's original "never introduce itself with a name" rule** — real-world reasoning: normal human phone-answering etiquette says the business name AND a personal name ("Steve's Mowing, this is Sarah") to make the call feel personal, not just transactional. The original no-name rule existed to avoid the AI pretending to be a specific real human who doesn't exist — but that concern is properly resolved by pairing the name with clear, upfront AI disclosure instead of removing personalization entirely.

**Researched first, not assumed:** confirmed Australia has no AI-specific disclosure requirement for *inbound* calls as of 2026 (existing AI voice disclosure rules mostly govern outbound telemarketing) — so this is a deliberate trust decision, not a legal requirement. Worth doing anyway, since it's simply the honest, right thing to do, and other voice AI platforms researched (e.g. Thoughtly) follow a similar "identify as AI" practice as standard.

**Final greeting, decided:** *"[Good morning/afternoon/evening], you've reached [Business Name]. I'm Sarah, the AI Office Manager. How can I help you today?"*

- **Name used:** "Sarah" — same internal name already established for the tradie-facing experience (§32a), now also used customer-facing, but always paired with explicit "AI" framing so no caller is left believing they spoke with an actual human named Sarah
- **"AI Office Manager"** — ties back to the product's original positioning language, reinforces transparency in the same breath as the name
- **Time-of-day greeting must be genuinely dynamic** — checks the actual time of the call (morning/afternoon/evening), not a hardcoded value. A fixed "Morning" said at 3pm would sound obviously wrong and undermine the natural, human feel this greeting is trying to achieve.
